...
<?php
	...
	$sqlx = mysqli_query($process->cnt,"SELECT * FROM tbtransaksiproduk WHERE NoTransaksi = '$nt' AND StatusTransaksi = 'Sudah Order'");
	$resx = mysqli_num_rows($sqlx);
	($resx == 0) ? header('Location:404notfound.php') : ""; 

	$KodeUser = $_COOKIE['KodeUser'];
	$sqlx = mysqli_query($process->cnt,"SELECT * FROM tbtransaksiproduk WHERE KodePelanggan = '$KodeUser' AND StatusTransaksi = 'Sudah Order'");
...
<body class="bg-light">
	...
	<div class="super_container">
		<div class="container" style="margin-top: 20px;background-color: #fff;padding: 20px;">
			<form action="../process/handler.php?action=tambahpembayaran" method="post" enctype="multipart/form-data">
				<h3>Pembayaran online</h3>
				<hr>
				<input autocomplete="off" type="hidden" class="form form-control" name="NoTransaksi" value="<?= $nt ?>"><br>
	          	<p>
				...
				<label><i class="fa fa-dollar-sign"></i> No Rekening Anda</label><br>
				...
				<small>( Total harga dari No Transaksi <?php 
					$sqlz = mysqli_query($process->cnt,"SELECT * FROM tbtransaksiproduk WHERE KodePelanggan = '$KodeUser' AND StatusTransaksi = 'Sudah Order'");
					$text2 = 0;
					while($resz = mysqli_fetch_array($sqlz)){
						$text2 += $resz['TotalHarga'];
						$NoTransaksii = $resz['NoTransaksi'];
						$KodeExpedisi = $resz['KodeExpedisi'];
					}
					$sqly = mysqli_query($process->cnt,"SELECT * FROM tbexpedisi WHERE KodeExpedisi = '$KodeExpedisi'");
					$resy = mysqli_fetch_array($sqly);
					$text2 += $resy['Harga'];

					echo $NoTransaksii." adalah <b>RP ".number_format($text2)."</b>";
				?> )</small><br>
				<strong>Transfer tepat sampai 3 digit terakhir !</strong><br>
				contoh:
				<img src="../public/img/format-transfer.jpg" alt=""><br>
				<input autocomplete="off" type="number" class="form form-control numberonly" name="JumlahPembayaran" placeholder="Jumlah Pembayaran" required style="margin-top: 10px;"><br>
				...
				<div class="buttonDiv" align="right">
					...
					<a href="tagihan.php?nt=<?= $nt ?>"><button type="button" class="btn btn-danger">Batal</button></a>
				</div>
			</form>
		</div>
	</div>
